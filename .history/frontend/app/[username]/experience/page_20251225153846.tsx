...
import useGetٍطs from "@/hooks/use-get-ٍطs";
import { ٍط } from "@/types/ٍط";
...
const ٍطsPage: React.FC = () => {
  const { data: ٍطs, isLoading, isError } = useGetExperience();
  ...
  const [selectedٍط, setSelectedٍط] = useState<ٍط | null>(null);
  ...
  const openEdit = (ٍط: Experience) => {
    setSelectedٍط(ٍط);
    setForm({
      title: ٍط.title,
      description: ٍط.description,
      company: ٍط.company,
      technologies: ٍط.technologies,
      startDate: ٍط.startDate,
      endDate: ٍط.endDate,
    });
    setIsEditOpen(true);
  };

  const openDelete = (ٍط: ٍط) => {
    setSelectedٍط(ٍط);
      ...
      reader.onloadend = () => {
        updateٍط.mutate(
          ...
          },
          {
            onSuccess() {
              setIsEditOpen(false);
              queryClient.invalidateQueries({ queryKey: ["ٍطs"] });
            },
          }
        );
      };
  ...
  const handleDelete = () => {
    if (!selectedٍط) return;
    deleteٍط.mutate(selectedٍط._id, {
      onSuccess() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      {!isLoading && !isError && ٍطs?.length === 0 && (
        ...
        <Dialog key={idx} open={open} onOpenChange={setOpen}>
          <DialogContent className="w-full max-w-full sm:max-w-md">
            ...
            {title !== "Confirm Delete" ? (
              ...
              <p>Are you sure you want to delete {selectedٍط?.title}?</p>
            )}
